...
public class ItemByakugan extends ElementsNarutomodMod.ModElement {
	...
	private static final UUID RINNESHARINGAN_MODIFIER = UUID.fromString("c69907b2-2687-47ab-aca0-49898cd38463");
	...
	public static double getByakuganChakraUsage(EntityLivingBase entity) {
		...
		return is(stack) ? ItemDojutsu.isOwner(stack, entity) ? BYAKUGAN_CHAKRA_USAGE 
		 : BYAKUGAN_CHAKRA_USAGE * 2 : (Double.MAX_VALUE * 0.001d);
	}

	public static double getRokujuyonshoChakraUsage(EntityLivingBase entity) {
		ItemStack stack =ItemDojutsu.getWorn(entity);
		return is(stack) && ItemDojutsu.isOwner(stack, entity) ? ROKUJUYONSHO_CHAKRA_USAGE 
		 : (Double.MAX_VALUE * 0.001d);
	}

	public static double getKaitenChakraUsage(EntityLivingBase entity) {
		ItemStack stack =ItemDojutsu.getWorn(entity);
		return is(stack) && ItemDojutsu.isOwner(stack, entity) ? KAITEN_CHAKRA_USAGE 
		 : (Double.MAX_VALUE * 0.001d);
	}

	public static double getKushoChakraUsage(EntityLivingBase entity) {
		ItemStack stack =ItemDojutsu.getWorn(entity);
		return is(stack) && ItemDojutsu.isOwner(stack, entity) ? KUSHO_CHAKRA_USAGE 
		 : (Double.MAX_VALUE * 0.001d);
	}
	...
	public void initElements() {
		...
		this.elements.items.add(() -> new Base(enuma).setUnlocalizedName("byakuganhelmet").setRegistryName("byakuganhelmet").setCreativeTab(TabModTab.tab));
	}

	public static class Base extends ItemDojutsu.Base {
		public Base(ArmorMaterial material) {
			super(material);
		}
		...
		public String getArmorTexture(ItemStack stack, Entity entity, EntityEquipmentSlot slot, String type) {
			return isRinnesharinganActivated(stack) ? "narutomod:textures/byakurinnesharingan_helmet.png" : "narutomod:textures/byakuganhelmet.png";
		}
		...
		public void onUpdate(ItemStack itemstack, World world, Entity entity, int par4, boolean par5) {
			...
			if (!world.isRemote && entity instanceof EntityLivingBase && entity.ticksExisted % 20 == 0 && this.isOwner(itemstack, (EntityLivingBase) entity) && itemstack.hasTagCompound() && itemstack.getTagCompound().hasKey(TENSEIGANEVOLVEDTIME)) {
		...
		public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
			...
			if (slot == EntityEquipmentSlot.HEAD && isRinnesharinganActivated(stack)) {
				multimap.put(SharedMonsterAttributes.MAX_HEALTH.getName(), new AttributeModifier(RINNESHARINGAN_MODIFIER, "byakurinnesharingan.maxhealth", 380d, 0));
			}
			return multimap;
		}
		...
		public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
			...
			if (stack.hasTagCompound()) {
				...
				if (d > 0.0d) {
					tooltip.add(I18n.translateToLocal(TextFormatting.AQUA + I18n.translateToLocal("tooltip.byakugan.tenseigantime") + (long) (d / 20d) + TextFormatting.WHITE));
				}
			}
		}
	...
	public static boolean wearingAny(EntityLivingBase entity) {
		return is(ItemDojutsu.getWorn(entity));
	}
	...
	public static boolean wearingRinnesharingan(EntityPlayer player) {
		...
		return is(itemstack) && isRinnesharinganActivated(itemstack);
	}
